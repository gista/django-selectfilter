...
var SelectFilter = {
    init: function(field_id, field_name, is_stacked, admin_media_prefix) {
        ...
        // create filter for chosen elements
        var filter_p_chosen = quickElement('p', selector_chosen, '');
        filter_p_chosen.className = 'selector-filter';
        var search_filter_label_chosen = quickElement('label', filter_p_chosen, '', 'for', field_id + "_input", 'style', 'width:16px;padding:2px');
        var search_selector_img_chosen = quickElement('img', search_filter_label_chosen, '', 'src', admin_media_prefix + 'img/admin/selector-search.gif');
        search_selector_img_chosen.alt = gettext("Filter");
        ...
        addEvent(selector_filter, 'keyup', function(e) { SelectFilter.filter_chosen_key_up(e, field_id); });
        addEvent(selector_filter, 'keydown', function(e) { SelectFilter.filter_chosen_key_down(e, field_id); });
    ...
    filter_chosen_key_up: function(event, field_id) {
        from = document.getElementById(field_id + '_to');
        // don't submit form if user pressed Enter
        if ((event.which && event.which == 13) || (event.keyCode && event.keyCode == 13)) {
            from.selectedIndex = 0;
    ...
    filter_chosen_key_down: function(event, field_id) {
        from = document.getElementById(field_id + '_to');
        // down arrow -- wrap around
        if ((event.which && event.which == 40) || (event.keyCode && event.keyCode == 40)) {
            from.selectedIndex = (from.length == from.selectedIndex + 1) ? 0 : from.selectedIndex + 1;
